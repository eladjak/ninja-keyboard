...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex md:w-[280px] md:flex-col border-ie h-dvh sticky top-0 sidebar-gradient">
      {/* Brand Header */}
      <div className="p-5 pb-4">
        <div className="flex items-center gap-3">
          <div className="flex size-11 items-center justify-center rounded-xl ninja-gradient shadow-md">
            <KiMascot size="small" mood="idle" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-foreground">נינג&apos;ה מקלדת</h1>
            <p className="text-xs text-muted-foreground">אימון הקלדה בעברית</p>
          </div>
        </div>

        {/* XP & Level Bar */}
        <div className="mt-4 rounded-lg border bg-card/50 p-3 space-y-2">
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-3 space-y-4" aria-label="ניווט ראשי">
        {navGroups.map((group) => (
          <div key={group.label}>
            <span className="mb-1 block px-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70">
              {group.label}
            </span>
